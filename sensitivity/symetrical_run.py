...
class Station(): 
    ...
    def _detection_capability(self, obs_attr: str, sim_attr: str, start_date: str, end_date: str, min_obs_threshold: float):
        ...
        Args:
            obs_attr (str): Attribute name for observed data.
            sim_attr (str): Attribute name for simulated data.
            min_obs_threshold (float): Min precipitation (mm/day) at/above which BOTH
                observed and simulated values count as "rain", used to classify
                hits/false alarms/misses/correct negatives.
    
        Returns:
            tuple: FBI, FAR, POD and Accuracy values as floats.

        CHANGED (symmetric threshold fix): previously, min_obs_threshold was applied
        only to sim_data, while obs_data used a fixed >0 / ==0 split regardless of
        the threshold passed in. That meant "raising the threshold" only made it
        harder for the *simulated* product to register a hit, while what counted as
        an observed rain day never moved - which is very likely why FBI and POD
        dropped almost uniformly across every product/station when the threshold
        was raised from 1.0 to 2.5 mm/day in the earlier sensitivity test: it was
        largely a mechanical consequence of this asymmetry, not a real precipitation
        signal. Both obs_data and sim_data are now compared against the SAME
        min_obs_threshold below.

        The previous asymmetric logic is left commented out beneath the new one in
        case you want to reproduce or compare against the old numbers.
        """
        ...
        # --- NEW: symmetric threshold applied to BOTH obs and sim ---
        hits = ((obs_data >= min_obs_threshold) & (sim_data >= min_obs_threshold)).sum()
        false_alarms = ((obs_data < min_obs_threshold) & (sim_data >= min_obs_threshold)).sum()
        misses = ((obs_data >= min_obs_threshold) & (sim_data < min_obs_threshold)).sum()
        correct_negatives = ((obs_data < min_obs_threshold) & (sim_data < min_obs_threshold)).sum()

        # --- OLD (asymmetric) logic, kept for reference/rollback ---
    ...
    def _extreme_indices(self, self_attr:str, start_date: str, end_date: str, wet_threshold: float = 1.0):
        ...
        ** Functions and classess later are created later to account for this
        ...
        Args:
            ...
            start_date (str) : Start date of the period.
            end_date (str)   : End date of the period.
            wet_threshold (float): precipitation (mm/day) at/above which a day counts
                as "wet" for CDD/CWD. Pass the same value as min_obs_threshold used in
                _detection_capability (via Generator.min_threshold) to keep the two
                analyses coherent. Defaults to 1.0 (ETCCDI convention).
        ...
        Calculated parameters:
            ...
            *Percentile Indices*  (independent of wet_threshold)
