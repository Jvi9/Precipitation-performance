# -*- coding: utf-8 -*-
"""
GPM-family-only version of figure_detection_elevation_scatter.py: CSI (a),
FBI (b), FAR (c), POD (d) vs. elevation, restricted to GPM-IMERGF, GPM-GWR,
and GPM-EXP only (Rain4PE/PISCO excluded) - same station-level scatter, no
binning, LOESS trend per product, Spearman rho printed and interpreted.

Useful when the GPM family's own differences need to be visible without
Rain4PE/PISCO's much higher performance compressing the y-axis scale.
"""
import os
import numpy as np
import pandas as pd
...
from configurations import DISPLAY_NAMES, PRODUCT_COLORS
...
print("GPM FAMILY ONLY (GPM-IMERGF, GPM-GWR, GPM-EXP) - Rain4PE/PISCO excluded.")
print("="*90)
...
print("\n" + "="*90)
print("PER-PRODUCT TREND STRENGTH AND SIGNIFICANCE, BY DETECTION METRIC (GPM FAMILY ONLY)")
print("="*90)
...
fig.suptitle('Relationship between elevation and detection performance metrics\n'
             '(GPM family only - points = individual stations, lines = LOESS trend, boxed values = Spearman \u03c1)',
             fontsize=19, y=1.01)

out_path = os.path.join(out_dir, 'figure_detection_elevation_scatter_gpm_only.png')
